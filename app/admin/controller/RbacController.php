...
class RbacController extends AdminBaseController
{
    ...
    public function role_add_post()
    {
        if ($this->request->isPost()) {
            ...
            if ($result !== true) {
                ...
                $result = Db::name('role')->insert(['status' => $data['status'], 'name' => $data['name'],'remark' => $data['remark'], 'create_time' => time()]);
                if ($result) {
                    //写日志
                    service('log', 'insert', ['2', '增加角色', session('admin_id'), serialize($this->request->param())]);
    ...
    public function role_edit_post()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($result !== true) {
                ...
                if (Db::name('role')->where('id',$id)->update(['status' => $data['status'], 'name' => $data['name'],'remark' => $data['remark'], 'update_time' => time()]) !== false) {
                    //写日志
                    service('log', 'insert', ['2', '编辑角色', session('admin_id'), serialize($this->request->param())]);
    ...
    public function role_delete()
    {
        ...
        if ($count > 0) {
            ...
            if (!empty($status)) {
                //写日志
                service('log', 'insert', ['2', '删除角色', session('admin_id'), serialize($this->request->param())]);
    ...
    public function authorize_post()
    {
        if ($this->request->isPost()) {
            ...
            if (is_array($this->request->param('menuId/a')) && count($this->request->param('menuId/a')) > 0) {
                ...
                //写日志
                service('log', 'insert', ['2', '设置权限', session('admin_id'), serialize($this->request->param())]);
                $this->success("授权成功！");
            } else {
                //当没有数据时，清除当前角色授权
