;(function () {
    ...
    //所有的发布操作，删除数据后刷新页面
        if ($('a.js-ajax-sync').length) {
            Wind.css('artDialog');
            Wind.use('artDialog', 'noty', function () {
                $('.js-ajax-sync').on('click', function (e) {
                    e.preventDefault();
                    var $_this  = this,
                        $this   = $($_this),
                        href    = $this.data('href'),
                    ...
                    art.dialog({
                        title: false,
                        icon: 'question',
                        content: msg ? msg : '发布后将不可修改，确定要发布吗？',
                        follow: $_this,
                        close: function () {
                            $_this.focus(); //关闭时让触发弹窗的元素获取焦点
                            return true;
            ...
            Wind.use('artDialog', 'noty', function () {
                $('.js-ajax-cancel').on('click', function (e) {
                    ...
                    art.dialog({
                        title: false,
                        icon: 'question',
                        content: msg ? msg : '取消后将不可显示，确定要取消吗？',
                        follow: $_this,
                        close: function () {
                            $_this.focus(); //关闭时让触发弹窗的元素获取焦点
                            return true;
...
function openUploadDialog(dialog_title, callback, extra_params, multi, filetype, app) {
    ...
    Wind.use("artDialog", "iframeTools", function () {
        art.dialog.open(GV.ROOT + 'admin/Asset/webuploader?' + params, {
            title: dialog_title,
            id: new Date().getTime(),
            width: '600px',
            height: '350px',
 ...
 * @param app  应用名,CMF的应用名
 */
function uploadOneImages(dialog_title, input_selector, extra_params, app) {
    openUploadDialog(dialog_title, function (dialog, files) {
        $(input_selector).val(files[0].filepath);
